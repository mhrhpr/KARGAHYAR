...
export default function Dashboard(){
  const [name,setName]=useState("مدیر کارگاه"); const [projects,setProjects]=useState(0); const [workers,setWorkers]=useState(0); const [contractors,setContractors]=useState(0); const [cost,setCost]=useState(0); const [pending,setPending]=useState(0);
  useEffect(()=>{(async()=>{const {data:user}=await supabase.auth.getUser();setName(user.user?.user_metadata?.full_name||user.user?.email?.split("@")[0]||"مدیر کارگاه");
    const [p,w,c,r,ops]=await Promise.all([
      ...
      supabase.from("worker_entries").select("payable"),
      syncPendingOperations()
    ]);
    setProjects(p.count||0);setWorkers(w.count||0);setContractors(c.count||0);setCost((r.data||[]).reduce((sum,row)=>sum+Number(row.payable||0),0));setPending(ops.pending);
  })()},[]);
  ...
  return <SafeAreaView style={{flex:1,backgroundColor:BRAND.bg}}><ScrollView contentContainerStyle={{padding:20,paddingBottom:40}}>
    ...
    <View style={{flexDirection:"row",gap:10,marginTop:10}}><Card label="پیمانکار" value={contractors}/><Card label="هزینه دستمزد ثبت‌شده" value={cost.toLocaleString("fa-IR")+" تومان"} accent/></View>
