...
export default function Materials(){
  ...
  return <SafeAreaView style={{flex:1,backgroundColor:BRAND.bg,padding:20}}><ScrollView contentContainerStyle={{paddingBottom:40}}>
    <Pressable onPress={()=>router.back()}><Text style={{color:BRAND.muted}}>← بازگشت</Text></Pressable><Text style={{color:BRAND.text,fontSize:29,fontWeight:"900" as const,marginTop:12}}>مصالح</Text>
    ...
    <View style={{gap:10,marginTop:20}}>{items.map(item=>{const low=Number(item.current_stock)<=Number(item.reorder_point);return <View key={item.id} style={{backgroundColor:BRAND.surface,borderWidth:1,borderColor:low?"#7A4A26":BRAND.border,borderRadius:18,padding:17}}>
      <View style={{flexDirection:"row",justifyContent:"space-between"}}><Text style={{color:BRAND.text,fontSize:18,fontWeight:"900" as const}}>{item.name}</Text><Text style={{color:low?"#F2B56B":"#7DD3A7",fontWeight:"800" as const}}>{low?"⚠️ نیاز به خرید":"موجودی مناسب"}</Text></View>
      ...
      <View style={{flexDirection:"row",gap:8,alignItems:"center",marginTop:12}}><TextInput value={consumeQty} onChangeText={setConsumeQty} keyboardType="numeric" placeholder="مقدار مصرف" placeholderTextColor="#707986" style={{flex:1,backgroundColor:BRAND.bg,color:BRAND.text,padding:12,borderRadius:12,borderWidth:1,borderColor:BRAND.border}}/><Pressable onPress={()=>consume(item)} style={{backgroundColor:BRAND.accent,padding:13,borderRadius:12}}><Text style={{color:"#fff",fontWeight:"900" as const}}>ثبت مصرف</Text></Pressable></View>
    </View>})}</View>
