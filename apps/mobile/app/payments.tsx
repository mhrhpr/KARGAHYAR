...
export default function Payments(){
  ...
  return <SafeAreaView style={{flex:1,backgroundColor:BRAND.bg,padding:20}}><ScrollView contentContainerStyle={{paddingBottom:40}}>
    <Pressable onPress={()=>router.back()}><Text style={{color:BRAND.muted}}>← بازگشت</Text></Pressable><Text style={{color:BRAND.text,fontSize:29,fontWeight:"900",marginTop:12}}>پرداخت‌ها</Text>
    <View style={{gap:9,marginTop:18}}><Text style={styles.label}>پرداخت به</Text><View style={{flexDirection:"row",gap:8}}>{[["worker","نیرو"],["contractor","پیمانکار"],["supplier","تأمین‌کننده"],["other","سایر"]].map(([v,t])=><Pressable key={v} onPress={()=>{setPartyType(v as PaymentOperation["partyType"]);setPartyId(null)}} style={{flex:1,padding:12,borderRadius:12,borderWidth:1,borderColor:partyType===v?BRAND.accent:BRAND.border,backgroundColor:partyType===v?BRAND.accent:BRAND.surface}}><Text style={{color:"#fff",textAlign:"center",fontSize:12,fontWeight:"800"}}>{t}</Text></Pressable>)}</View>
    {partyType!=="other"&&<View style={{gap:8,marginTop:4}}>{parties.map(p=><Pressable key={p.id} onPress={()=>setPartyId(p.id)} style={{padding:13,borderRadius:12,borderWidth:1,borderColor:partyId===p.id?BRAND.accent:BRAND.border,backgroundColor:BRAND.surface}}><Text style={{color:BRAND.text,fontWeight:"800"}}>{p.name}</Text></Pressable>)}</View>}
    ...
    <View style={{flexDirection:"row",gap:8}}>{[["cash","نقدی"],["card","کارت"],["transfer","انتقال"],["other","سایر"]].map(([v,t])=><Pressable key={v} onPress={()=>setMethod(v as PaymentOperation["method"])} style={{flex:1,padding:12,borderRadius:12,borderWidth:1,borderColor:method===v?BRAND.accent:BRAND.border,backgroundColor:method===v?BRAND.accent:BRAND.surface}}><Text style={{color:"#fff",textAlign:"center",fontSize:12,fontWeight:"800"}}>{t}</Text></Pressable>)}</View>
...
const styles=StyleSheet.create({label:{color:BRAND.muted,fontSize:12},input:{backgroundColor:BRAND.surface,color:BRAND.text,padding:15,borderRadius:13,borderWidth:1,borderColor:BRAND.border},primary:{backgroundColor:BRAND.accent,padding:15,borderRadius:13},primaryText:{color:"#fff",textAlign:"center",fontWeight:"900"},card:{backgroundColor:BRAND.surface,borderWidth:1,borderColor:BRAND.border,borderRadius:17,padding:17},title:{color:BRAND.text,fontWeight:"800"},amount:{color:BRAND.text,fontWeight:"900"},sub:{color:BRAND.muted,marginTop:5}};
