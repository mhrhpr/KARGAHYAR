...
export default function Projects(){
  ...
  return <SafeAreaView style={{flex:1,backgroundColor:BRAND.bg,padding:20}}><ScrollView contentContainerStyle={{paddingBottom:40}}>
    <View style={{flexDirection:"row",justifyContent:"space-between",alignItems:"center"}}><Text style={{color:BRAND.text,fontSize:28,fontWeight:"900"}}>پروژه‌های من</Text><Pressable onPress={()=>router.push("/project-wizard")}><Text style={{color:BRAND.accent,fontWeight:"900"}}>+ پروژه</Text></Pressable></View>
    {projects.length===0?<View style={{marginTop:20,borderRadius:18,borderWidth:1,borderColor:BRAND.border,padding:20,backgroundColor:BRAND.surface}}><Text style={{color:BRAND.text,fontWeight:"800",fontSize:18}}>هنوز پروژه‌ای ثبت نشده است.</Text><Text style={{color:BRAND.muted,marginTop:8}}>اولین پروژه را در کمتر از یک دقیقه بساز.</Text><Pressable onPress={()=>router.push("/project-wizard")} style={{marginTop:16,backgroundColor:BRAND.accent,padding:14,borderRadius:13}}><Text style={{color:"#fff",textAlign:"center",fontWeight:"900"}}>ساخت پروژه جدید</Text></Pressable></View>
